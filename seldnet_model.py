...
class MSELoss_ADPIT(object):
    def __init__(self):
        super().__init__()
        self._each_loss = nn.MSELoss(reduction='mean')# we have changed the loss function from MSE loss to cross entropy loss
    ...
    def __call__(self, output, target):
        ...
        print("target and output", target, output)
    ...
    def __init__(self, in_feat_shape, out_shape, params):
        ...
        if len(params['f_pool_size']):
            for conv_cnt in range(len(params['f_pool_size'])):
                ...
                print("xxxxxx",params['nb_cnn2d_filt'] if conv_cnt else in_feat_shape[1])
                print(conv_cnt, in_feat_shape[1]-3)
                # exit()
