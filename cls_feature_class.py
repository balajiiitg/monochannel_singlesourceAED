...
class FeatureClass:
    def __init__(self, params, is_eval=False):
        ...
        if self._use_salsalite and self._dataset == 'mic':
            ...
            self._upper_bin = np.int64(
                np.floor(np.min((params['fmax_doa_salsalite'], self._fs // 2)) * self._nfft / np.float64(self._fs)))
    ...
    def _load_audio(self, audio_path):
        ...
        print(len(audio))
        audio = [audio, np.zeros(len(audio)), np.zeros(len(audio)), np.zeros(len(audio))]# here we are considering audio
        # audio = [audio]
        audio = np.transpose(audio)
        print(np.shape(np.transpose(audio)))
    ...
    def _get_spectrogram_for_file(self, audio_filename):
        ...
        print("pppp", nb_feat_frames)
        print("xxx", nb_label_frames)
        ...
        for frame_ind, active_event_list in _desc_file.items():
            if frame_ind < _nb_label_frames:
                for active_event in active_event_list:
                    # print("active event list", active_event_list)
                    # exit()
                    se_label[frame_ind, active_event[0]] = 1
                    x_label[frame_ind, active_event[0]] = active_event[0]
                    y_label[frame_ind, active_event[0]] = active_event[0]
                    z_label[frame_ind, active_event[0]] = active_event[0]
                ...
                for i, active_event in enumerate(active_event_list):
                    ...
                    if i == len(active_event_list) - 1:  # if the last
                        if len(active_event_list_per_class) == 1:  # if no ov from the same class
                            ...
                            # print("event shape", active_event_a0)
                            # x_label[frame_ind, 0, active_event_a0[0]] = active_event_a0[1]
                            # y_label[frame_ind, 0, active_event_a0[0]] = active_event_a0[1]
                            # z_label[frame_ind, 0, active_event_a0[0]] = active_event_a0[1]
    ...
    def extract_all_labels(self):
        ...
        for sub_folder in os.listdir(self._desc_dir):
            ...
            for file_cnt, file_name in enumerate(os.listdir(loc_desc_folder)):
                ...
                # print("labels", label_mat.shape)
                # # np.savetxt('test_label.out', )
                # exit()
    ...
    def regression_label_format_to_output_format(self, _sed_labels, _doa_labels):# regression function for DOAA
        """
        ...
        # print("number of classes", _nb_classes)
        # exit()
